...
// ── 메모리 캐시 ────────────────────────────
const memCache  = new Map();   // id → children[]
const pending   = new Map();   // id → Promise (중복 요청 방지)
let   busy      = false;       // Playwright 동시 실행 방지
const waitQueue = [];          // 순서 보장 큐
...
function writeCache(id, data) {
  try {
    fs.writeFileSync(cacheFile(id), JSON.stringify({ cachedAt: new Date().toISOString(), data }), 'utf-8');
...
async function fetchFromSite(categoryId) {
  return withBrowser(async () => {
    ...
    try {
      const url = `https://itemscout.io/category/${categoryId}`;
      console.log(`[CatFetch] ${url}`);
      await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 20000 });
      await page.waitForTimeout(2000); // React 렌더링 대기
